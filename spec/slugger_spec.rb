...
describe "Slugger class" do
  ...
  describe "top_league_player_in_category_for_year" do
    before do
      players_file = File.expand_path '../data/players.csv', __FILE__
      stats_file = File.expand_path '../data/stats.csv', __FILE__
      @slugger = Slugger.new(players_file, stats_file)
    ...
    describe "with a valid category and sufficient at-bats" do
      it "returns the correct person id" do
        assert_equal "abadijo01", @slugger.top_league_player_in_category_for_year(2008, "NL", :home_runs, 10)
        assert_equal "aaronto01", @slugger.top_league_player_in_category_for_year(2008, "AL", :at_bats, 10)
      end
    end
    describe "with a valid category but the wrong league" do
      it "returns nil" do
        assert_equal "aaronto01", @slugger.top_league_player_in_category_for_year(2008, "AL", :home_runs, 10)
      end
    end
    describe "with a valid category but insufficient at-bats" do
      it "returns nil" do
        assert_nil @slugger.top_league_player_in_category_for_year(2008, "NL", :home_runs, 1000)
      end
    end
    describe "with an invalid category" do
      it "returns nil" do
        assert_nil @slugger.top_league_player_in_category_for_year(2008, "NL", :not_a_valid_category, 50)
      end
    end
  end
